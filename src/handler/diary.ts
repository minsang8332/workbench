...
ipcMain.handle('diary:read', async (event, payload: IpcPayload.Diary.IRead) => {
    ...
    try {
        ...
        response.error = handlerTool.createError(e as Error)
    }
    return response
})
// 문서 저장
ipcMain.handle(
    'diary:write',
    async (event, payload: IpcPayload.Diary.IWrite) => {
        let response: IpcHandle.IResponse = handlerTool.createResponse()
        try {
            ...
            fs.writeFileSync(
                path.format({ dir: target, name: filename, ext }),
                text
            )
            response.data = {
                writed: filename,
...
ipcMain.handle(
    'diary:write-dir',
    async (event, payload: IpcPayload.Diary.IWriteDir) => {
        let response: IpcHandle.IResponse = handlerTool.createResponse()
        try {
            let target = path.join(rootDir, payload.target)
            if (!(await isSubdir(rootDir, target))) {
...
ipcMain.handle(
    'diary:remove',
    async (event, payload: IpcPayload.Diary.IRemove) => {
        let response: IpcHandle.IResponse = handlerTool.createResponse()
        try {
