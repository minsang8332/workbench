...
ipcService.controller(
    IPC_DIARY.LOAD,
    async (request: IpcController.IRequest.Diary.ILoad, response: IpcController.IResponse) => {
...
ipcService.controller(
    IPC_DIARY.READ,
    (request: IpcController.IRequest.Diary.IRead, response: IpcController.IResponse) => {
        let text = null
...
ipcService.controller(
    IPC_DIARY.WRITE,
    async (request: IpcController.IRequest.Diary.IWrite, response: IpcController.IResponse) => {
...
ipcService.controller(
    IPC_DIARY.WRITE_DIR,
    async (request: IpcController.IRequest.Diary.IWriteDir, response: IpcController.IResponse) => {
        let target = path.join(rootDir, request.target)
        if (!(await isSubdir(rootDir, target))) {
            throw new Error('유효하지 않은 경로 입니다.')
        }
...
ipcService.controller(
    IPC_DIARY.REMOVE,
    async (request: IpcController.IRequest.Diary.IRemove, response: IpcController.IResponse) => {
...
ipcService.controller(
    IPC_DIARY.RENAME,
    async (request: IpcController.IRequest.Diary.IRename, response: IpcController.IResponse) => {
        let target = request.target
        if (!_.isString(target)) {
            target = '/'
        }
...
ipcService.controller(
    IPC_DIARY.MOVE,
    async (request: IpcController.IRequest.Diary.IMove, response: IpcController.IResponse) => {
        let target = request.target
        if (!_.isString(target)) {
            target = '/'
        }
